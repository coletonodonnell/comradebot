...
class Music(commands.Cog):
  ...
  def __init__(self, client):
    self.client = client
    self.client.loop.create_task(self.queue_system())
    self.song_queue = asyncio.Queue()
    self.next_queue = asyncio.Event() 
  ...
  async def queue_system(self):
    while True: # While loop
      self.next_queue.clear() # Set the flag to false
      task = await self.song_queue.get() # Fetch most recent task (this is an array, where the 0th term is the voice_client object, and the second is the path to the video)
      connected = task[0] # Fetch the 1st element
      path = task[1] # Fetch the 2nd element
      await self.player_queue(connected, path) # Throw these into player_queue()
      await self.next_queue.wait() # The flag must be True for the next iteration to continue, this flag is set in player_queue()
  ...
  async def player_queue(self, connected, path):
    ...
    if os.path.exists(path):
      os.remove(path)
    self.song_queue.task_done() # This tells the queue that the task has finished completely, and that it can be removed from the dictionary
    self.next_queue.set() # This sets the next_queue() to True.
  ...
  async def play(self, ctx, link):
    ...
    await self.song_queue.put([connected, download]) # Parse it into the queue
  ...
  async def clear(self, ctx):
    if Music.is_voice_connected(self, ctx) == True:
      connected = ctx.voice_client
      connected.stop()
      for x in range(self.song_queue.qsize()):
        connected.stop()
